...
namespace StockifyWeb
{
    ...
    public static class NotificationService
    {
        ...
        {
            // PRIMERO: Agregar notificación al sistema (en la sesión actual)
            var mensaje = $"Producto '{nombreProducto}' registrado exitosamente";
            AgregarNotificacion(mensaje, "success", "fa-box");

            // SEGUNDO: Enviar correo electrónico de forma asíncrona (sin bloquear)
            // El correo se envía en segundo plano, pero la notificación ya está guardada
        ...
        // Método genérico para notificar con correo
        public static void NotificarConCorreo(string mensaje, string tipo, string icono, string asuntoCorreo, string cuerpoCorreo)
        {
            // PRIMERO: Agregar a la sesión
            AgregarNotificacion(mensaje, tipo, icono);

            // SEGUNDO: Enviar correo en segundo plano
            System.Threading.Tasks.Task.Run(() =>
            {
                EnviarCorreo(asuntoCorreo, cuerpoCorreo);
            });
        }

        // Métodos adicionales para diferentes tipos de notificaciones

        public static void NotificarActualizacionProducto(string nombreProducto)
        {
            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                var asunto = "Producto Actualizado - Stockify";
                var cuerpo = GenerarCuerpoCorreo(
                    "Producto Actualizado",
                    $"El producto '{nombreProducto}' ha sido actualizado en el sistema."
                );
                EnviarCorreo(asunto, cuerpo);
            });
        }

        public static void NotificarStockBajo(string nombreProducto, int cantidadActual)
        {
            var mensaje = $"⚠️ Stock bajo: '{nombreProducto}' tiene solo {cantidadActual} unidades";
            AgregarNotificacion(mensaje, "warning", "fa-exclamation-triangle");

            System.Threading.Tasks.Task.Run(() =>
            {
                var asunto = "⚠️ Alerta de Stock Bajo - Stockify";
                var cuerpo = GenerarCuerpoCorreo(
                    "Alerta de Stock Bajo",
                    $"El producto '{nombreProducto}' tiene solo {cantidadActual} unidades disponibles. Se recomienda reabastecer.",
                    "#ffc107"
                );
                EnviarCorreo(asunto, cuerpo);
            });
        ...
        public static void NotificarNuevaOrden(string tipoOrden, string numeroOrden)
        {
            var mensaje = $"Nueva {tipoOrden} #{numeroOrden} creada";
            AgregarNotificacion(mensaje, "success", "fa-file-invoice");

            System.Threading.Tasks.Task.Run(() =>
            {
                var asunto = $"Nueva {tipoOrden} - Stockify";
                var cuerpo = GenerarCuerpoCorreo(
                    $"Nueva {tipoOrden}",
                    $"Se ha creado la {tipoOrden} #{numeroOrden} en el sistema."
                );
                EnviarCorreo(asunto, cuerpo);
            });
        }
        ...
        private static string GenerarCuerpoCorreo(string titulo, string contenido, string colorBorde = "#8aa2ff")
        {
            return $@"
                <html>
                <body style='font-family: Arial, sans-serif;'>
                    <div style='max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f4f4f4;'>
                        <div style='background-color: white; padding: 30px; border-radius: 10px;'>
                            ...
                            <p style='color: #666; font-size: 16px;'>{contenido}</p>
                            <div style='background-color: #f8f9fa; padding: 15px; border-left: 4px solid {colorBorde}; margin: 20px 0;'>
